...
def handle_arguments():
    ...
    parser.add_argument(
        "--session", type=str, nargs=2, required=False, help="if used, requires date and sessionid string, and performs ROICaT on single session"
    )
...
def define_dirs_within(args, ses):
    ...
    for plane_name in ses.value["planeNames"]:
        print(plane_name)
...
def main():
    ...
    if pipeline_type == "tracking_across":
        ...
        pipeline = pipeline_tracking(
            params, feature_additional=[tensor(roiActivity)], additional_feature_name=["fluorescence"]
