...
class pipeline_tracking:
    def __init__(self, params: dict, feature_additional=None, additional_feature_name=None):
        ...
        if self.params['data_loading']['data_kind'] == 'data_suite2p':
            assert self.params['data_loading']['dir_outer'] is not None, f"params['data_loading']['dir_outer'] must be specified if params['data_loading']['data_kind'] is 'data_suite2p'."
            ...
            assert len(paths_allDataObjs) == 1, f"ERROR: Found {len(paths_allDataObjs)} files matching the search pattern '{self.params['data_loading']['data_roicat']['filename_search']}' in '{params['data_loading']['dir_outer']}'. Exactly one file must be found."
    ...
    def compute_similarities(self, feature_additional=None, additional_feature_method=None):
        ...
        s_sf, s_NN, s_SWT, s_sesh, s_additional = self.sim.compute_similarity_blockwise(
            ...
            features_SWT=feature_additional[0], # self.swt.latents,  ## Scattering wavelet transform output latents
        ...
        self.sim.make_normalized_similarities(
            ...
            features_SWT=feature_additional[0], #self.swt.latents,  ## SWT latents
